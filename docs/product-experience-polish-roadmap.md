...
2026-06-24 补充：P110-P113 已在 P63 之后追加完成新的 UI 审美与布局重构闭环。P110 建立 Calm Command Center / Ledger Pro 方向，P111 选定高保真参考图作为视觉真源，P112 对全产品页面执行参考图细节修复、桌面/移动截图复验和子 agent 复审，P113 针对用户继续指出的布局错位、移动端溢出、卡片压缩、触控目标过小和 raw 内容直出问题完成布局高保真精修。P113 结论为 `pass`，全路由桌面/390px 移动 DOM layout 指标 issueCount 为 0；该结论不新增业务能力或后端契约。
...
目标：在 P112 后继续收敛用户可见的布局问题，尤其是移动端横向溢出、metric rail 裁切、桌面 report hero 压缩、过小操作目标、规则/设置 raw 内容直出和决策详情有数据状态层级不足。

验收：

- 全 19 个产品路由采集桌面与 390px 移动截图。
- 每页执行 no-overflow、clip/overlap、touch target、console health 和页面层级检查。
- 规则阈值与提案 raw 详情默认折叠，设置错误提示不再压过主 report hero。
- 决策详情同时验证空态和临时本地数据生成的 populated state。
- P113 不新增投资业务能力，不改变交易安全边界，不声称券商接口、自动交易、一键交易、代下单、外部推送、自动确认、自动规则应用、自动修复或收益承诺。

验收记录：`docs/release/acceptance/2026-06-24-p113-layout-fidelity-polish.md`。
