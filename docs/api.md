...
用途：读取本地每日自动运行配置和最近一次运行状态，供每日自动运行页面展示。该接口只读，不触发运行、不刷新数据、不执行交易、不自动确认或应用规则。

响应 `data` 至少包含 `enabled`、`run_time`、`timezone`、`scope`、`status` 和 `safety_note`。若存在最近一次运行记录，可包含 `run_id`、`idempotency_key`、`local_date`、`last_run_at`、`next_run_at`、`failure_code`、`failure_reason`、`latest_decision_link`、`latest_notification_link`、`latest_audit_link` 和 `missing_action`。
...
用途：根据当前本地账户快照和持仓标签计算核心、卫星、现金三类资产相对目标比例的偏离。该接口只生成本地复核结果和审计记录，不执行交易、不创建订单、不自动调整持仓。

请求：

```json
{
  "target_core_ratio": 0.60,
  "target_satellite_ratio": 0.30,
...
响应 `data` 至少包含 `review_id`、`review_date`、`total_assets`、`drift_threshold`、`items`、`audit_event_ids` 和 `safety_statement`。`items` 中每项包含 `bucket`、`target_ratio`、`actual_ratio`、`drift_ratio`、`target_value`、`actual_value`、`recommendation` 和 `manual_amount`。`recommendation` 只能作为人工复核建议，不代表系统交易或自动调仓。
...
用途：在真实复盘或风险处置发现 SOP 覆盖不足时，生成一个待人工治理的 SOP 增补提案。该接口只创建提案、通知和审计记录，不应用正式规则，不修改持仓，不执行交易。

请求：

```json
{
  "scenario_key": "liquidity_danger_repeat",
  "scenario_title": "流动性危险状态重复出现",
  "occurrence_count": 3,
  "sample_window": "2026-Q2"
}
```

响应 `data` 至少包含 `proposal_id`、`status`、`notification_id`、`audit_event_ids` 和 `safety_statement`。生成后的提案仍必须经过用户确认、效果验证、守门人审计和最终确认流程；不得因创建成功而自动应用规则。
