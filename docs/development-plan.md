...
每个阶段建议独立提交；P19–P88 已作为当前交付基线完成并归档。P88 已完成并归档，P86 后剩余 27 行中 17 行升级为 `real_pass`、10 行保留 `partial`；当前仍有 10 个 full-release-required rows 非 `real_pass`。不得声称 full original-requirement pass，除非所有 full-release-required rows 都成为 `real_pass` 或被有效重分类。
