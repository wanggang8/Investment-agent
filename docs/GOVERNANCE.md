...
P112 已归档到 `openspec/changes/archive/2026-06-24-p112-reference-fidelity-detail-pass/`。P112 已处理 P111 归档后用户复核指出的高保真细节差距，覆盖二级页面旧式结构、report hero 高度与主状态位置、侧栏分组密度、首页/工作台状态/图标/ledger 细节、移动端首屏效率和截图证据质量。P112 以同一张第二方案参考图为唯一视觉真源，完成 18 个产品路由桌面与 390px 移动截图、mismatch ledger、桌面/移动子 agent 复审；最终结论为 `pass_with_minor_notes`，无 P0/P1/P2 或 release-blocking 视觉问题。P112 不需要后端调整，不新增后端 API、SQLite schema、Eino workflow、LLM 能力、投资规则、数据源、Docker/安装/发布包/物理第二机器验收、券商接口、自动交易、一键交易、代下单、外部推送、自动确认、自动规则应用、自动修复、真实库覆盖或收益承诺。

P111 已归档到 `openspec/changes/archive/2026-06-24-p111-high-fidelity-reference-redesign/`。P111 已把用户确认的第二套参考图 `/Users/vick/.codex/generated_images/019ef8a7-f5c0-7442-95b9-e72bcdc89de1/ig_05724f56eb7089ab016a3b9109e1848191a87e68883d0c9826.png` 作为高保真视觉真源落到全产品：深色侧栏、白色顶部状态栏、report hero、priority action queue、status metric grid、snapshot strip、progress tracker、evidence checklist、ledger surfaces 和底部安全声明已统一；覆盖 18 个路由桌面 1492×1068 与移动 390×844 截图、visual mismatch ledger、contact sheet 和验收记录 `docs/release/acceptance/2026-06-24-p111-high-fidelity-reference-redesign.md`。P111 已修复用户指出的高保真差距：首屏右栏快照位置、侧栏多余运行卡片、CSS 画图标、首屏密度、`/data-quality` 首屏顺序和通用页面旧 hero 风格；当前无已知 P0/P1/P2 高保真视觉 mismatch。P111 不新增后端 API、SQLite schema、Eino workflow、LLM 能力、投资规则、数据源、Docker/安装/发布包/物理第二机器验收、券商接口、自动交易、一键交易、代下单、外部推送、自动确认、自动规则应用、自动修复、真实库覆盖或收益承诺。
...
P105 已归档到 `openspec/changes/archive/2026-06-24-p105-release-v0-1-1/`。P105 已将当前源码发布版本推进到 `v0.1.1`，同步 root `VERSION`、`web/package.json` 与 `web/package-lock.json`，更新 release-facing 材料，并生成 `docs/release/acceptance/2026-06-24-p105-v0.1.1-release.md`。P105 通过 OpenSpec、Go、前端 test/build、P104 runner、P92/P93 和 whitespace gates；P105 不新增投资运行时能力，不处理 Docker 验证、安装/升级/卸载验证、分发包刷新或物理第二机器复验，不声称 GitHub Release workflow 已成功完成；不声称券商接口、自动交易、一键交易、代下单、外部推送、自动确认、自动规则应用、收益承诺、登录源、付费源、授权源、Level2 或高频源。

P104 已归档到 `openspec/changes/archive/2026-06-24-p104-full-product-operation-linkage-acceptance/`。P104 已建立可重复的本地源码产品操作联动验收门禁，新增验收矩阵 `docs/release/acceptance/2026-06-24-p104-product-operation-linkage-matrix.md`、验收记录 `docs/release/acceptance/2026-06-24-p104-full-product-operation-linkage-acceptance.md`、runner `scripts/p104-product-operation-linkage-acceptance.sh` / `scripts/p104_product_operation_linkage_acceptance.py` 和证据包 `docs/release/ui-audit-assets/2026-06-24-p104-product-operation-linkage/`。Fresh runner 覆盖组合校准与一致性拒绝、持仓维护、线下流水、批量导入、修正审计、再平衡、人工计划确认、决策闭环、通知、风险、数据质量处置、dashboard/review/audit 下游读回；SQLite 读回确认 6 个组合快照、3 条交易流水、3 条确认、1 条风险处置、1 条通知 readback、1 条数据质量处置退休、15 条审计事件，且自动确认、broker/order/push 表和自动规则/交易审计均为 0。P104 不修改投资运行时能力，不处理 Docker、安装/升级/卸载、GitHub Release、分发包刷新或物理第二机器复验，不声称券商接口、自动交易、一键交易、代下单、外部推送、自动确认、自动规则应用、收益承诺、登录源、付费源、授权源、Level2 或高频源。

P103 已归档到 `openspec/changes/archive/2026-06-24-p103-product-acceptance-ux-linkage-fixes/`。P103 已修复 P102 产品验收发现的非阻断 UX 问题并补充联动验收记录，验收记录见 `docs/release/acceptance/2026-06-24-p103-product-acceptance-ux-linkage-fixes.md`：空组合首次进入 `/positions` 改为首次使用引导，决策详情真实模型分析材料默认折叠但保留可审计全文且摘要层截短，`/decision-loop?decision_id=<id>` 聚焦目标闭环记录。P103 明确本轮是 scoped linked-flow validation，不等于逐按钮/逐异常分支穷尽验收；全量原始需求与代码真实性仍以 P92/P93 为准。P103 验证通过 P103 OpenSpec、前端 182 tests、前端 build、Go tests、全量 OpenSpec、P92/P93 checks、`git diff --check` 和浏览器 rendered console health。P103 不修改投资规则、最终裁决逻辑、LLM prompt、SQLite schema 或 HTTP API 契约；不新增券商接口、自动交易、一键交易、代下单、外部推送、自动确认、自动规则应用、收益承诺、登录源、付费源、授权源、Level2 或高频源。

P102 已归档到 `openspec/changes/archive/2026-06-24-p102-product-acceptance-audit/`。P102 已完成真实模型恢复后的产品级本地源码验收，验收记录见 `docs/release/acceptance/2026-06-24-p102-product-acceptance-audit.md`，证据包见 `docs/release/ui-audit-assets/2026-06-24-product-acceptance-audit/`；结论为 `product_acceptance_pass_with_non_blocking_ux_findings`。P102 使用本地 Go backend、Vite frontend、SQLite/VecLite、`configs/config.yaml` 和真实 OpenAI-compatible `gpt-5.4-mini` 路径，完成全路由桌面/移动截图、组合校准与一致性拒绝、真实模型咨询、人工计划确认、决策闭环、审计、复盘、工作台、通知、API/SQLite/DOM readback；确认本地产品主链路可用，安全边界清晰。P102 遗留非阻断 UX findings：空组合首次进入 `/positions` 时通用 `读取失败` 易误导新用户，决策详情长文本密度较高，决策闭环 query 深链接未聚焦为单条详情。P102 不处理 Docker、安装/升级/卸载、GitHub Release、分发包刷新或物理第二机器复验，不声称券商接口、自动交易、一键交易、代下单、外部推送、自动确认、自动规则应用、收益承诺、登录源、付费源、授权源、Level2 或高频源。

P101 已归档到 `openspec/changes/archive/2026-06-24-p101-unify-local-config-path/`。P101 已完成本地配置路径统一与 OpenAI-compatible LLM 请求兼容加固：当前本地程序与 P63/P71/P72/P75 本地源码验收脚本默认都读取 ignored `configs/config.yaml`，保留脚本环境变量显式覆盖能力；LLM client 继续使用 `<base_url>/chat/completions` 和 `model/messages` body，但补充 JSON accept/user-agent headers、一次 timeout retry，并将默认/example timeout 调整为 60 秒。P101 已用用户配置的 OpenAI-compatible `gpt-5.4-mini` 完成真实 `llm-smoke`、P71、P72、P86、OpenSpec、Go、前端、P92/P93 和 whitespace gates，验收记录见 `docs/release/acceptance/2026-06-24-p101-unify-local-config-path.md`。P101 不提交或打印真实 key，不新增投资运行时能力，不声称券商接口、自动交易、一键交易、代下单、外部推送、自动确认、自动规则应用、收益承诺、登录源、付费源、授权源、Level2 或高频源。

P100 已归档到 `openspec/changes/archive/2026-06-23-p100-local-source-final-acceptance/`。P100 已完成本地源码最终验收，生成 `docs/release/acceptance/2026-06-23-p100-local-source-final-acceptance.md`，结论为 `local_source_release_acceptance_passed_with_documented_degradation`。P100 以 Go 后端、Vite 前端、本地 SQLite/VecLite、真实浏览器 UI、API/readback 和审计证据为准，复验 P92/P93 需求与代码真实性门禁通过，9 个主路由 × 3 视口产品设计 QA 通过，P90 capital-flow provider closure 通过；因本环境无真实 LLM key，P71/P72/P86 nested real-LLM-dependent rerun 仅记录 `authentication_or_key` 降级，不声称 fresh real LLM output；P89 legacy script 存在 P90 后 UI/provider 断言漂移，不作为 fresh pass 声明。P100 明确不处理 Docker、安装/升级/卸载、GitHub Release、Git tag、分发包刷新或物理第二机器复验，不新增投资运行时能力，不声称券商接口、自动交易、一键交易、代下单、外部推送、自动确认、自动规则应用、收益承诺、登录源、付费源、授权源、Level2 或高频源。
