...
TMP_DIR="$ROOT_DIR/tmp/e2e-smoke"
CONFIG_PATH="$TMP_DIR/config.yaml"
SQLITE_PATH="$TMP_DIR/investment-agent-smoke.db"
VECLITE_PATH="$TMP_DIR/veclite"
SERVER_LOG="$TMP_DIR/server.log"
WEB_LOG="$TMP_DIR/web.log"
SERVER_PORT="${E2E_SERVER_PORT:-18080}"
WEB_PORT="${E2E_WEB_PORT:-14173}"
...
VITE_API_PROXY_TARGET="http://127.0.0.1:$SERVER_PORT" bash -c 'cd "$1" && exec env VITE_API_PROXY_TARGET="$2" ./node_modules/.bin/vite --host 127.0.0.1 --port "$3"' _ "$ROOT_DIR/web" "http://127.0.0.1:$SERVER_PORT" "$WEB_PORT" >"$WEB_LOG" 2>&1 &
WEB_PID="$!"
...
E2E_BASE_URL="http://127.0.0.1:$WEB_PORT" npm --prefix "$ROOT_DIR/web" run test:e2e -- --config playwright.config.ts --workers=1 local-smoke.spec.ts p63-full-ui-regression.spec.ts
cleanup
SERVER_PID=""
WEB_PID=""
rm -rf "$TMP_DIR"
