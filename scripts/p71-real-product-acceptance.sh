...
TMP_DIR="$ROOT_DIR/tmp/p71-real-product-acceptance"
CONFIG_PATH="$TMP_DIR/config.p71.yaml"
LOCAL_CONFIG="${P71_LOCAL_CONFIG:-$ROOT_DIR/configs/config.local.yaml}"
SQLITE_PATH="$TMP_DIR/investment-agent-p71.db"
VECLITE_PATH="$TMP_DIR/veclite.json"
SERVER_LOG="$TMP_DIR/server.log"
WEB_LOG="$TMP_DIR/web.log"
SERVER_PORT="${P71_SERVER_PORT:-18091}"
WEB_PORT="${P71_WEB_PORT:-14191}"
SERVER_BIN="$TMP_DIR/server"
ARTIFACT_DIR="${P71_ARTIFACT_DIR:-$ROOT_DIR/docs/release/ui-audit-assets/2026-06-18-p71}"
...
require_log_contains() {
  ...
  if ! grep -Fq "$needle" "$file"; then
    echo "P71 required output missing: $needle" >&2
    echo "Log: $file" >&2
    exit 1
  fi
}
...
assert_real_acceptance_config() {
  ...
  if grep -Eq '^[[:space:]]*use_stub:[[:space:]]*true[[:space:]]*$' "$file"; then
    echo "P71 rejects mock-only acceptance config: data_sources.use_stub=true" >&2
    exit 1
  fi
  if ! grep -Eq '^[[:space:]]*enabled:[[:space:]]*true[[:space:]]*$' "$file"; then
    echo "P71 requires enabled real market collectors" >&2
    exit 1
  fi
  if ! grep -Fq 'csindex' "$file"; then
    echo "P71 requires csindex current-data collector" >&2
    exit 1
  fi
}

if [[ ! -f "$LOCAL_CONFIG" ]]; then
  echo "P71 requires a local config with a real LLM key: $LOCAL_CONFIG" >&2
  exit 1
fi
...
{
  echo "P71 precheck started at $(date -u +%Y-%m-%dT%H:%M:%SZ)"
...
P71_CAPTURE_SCREENSHOTS=1 \
P71_ARTIFACT_DIR="$ARTIFACT_DIR" \
E2E_BASE_URL="http://127.0.0.1:$WEB_PORT" \
npm --prefix "$ROOT_DIR/web" run test:e2e -- --config playwright.config.ts --workers=1 p71-real-product-acceptance.spec.ts
