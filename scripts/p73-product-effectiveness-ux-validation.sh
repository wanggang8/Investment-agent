...
TMP_DIR="$ROOT_DIR/tmp/p73-product-effectiveness-ux-validation"
CONFIG_PATH="$TMP_DIR/config.p73.yaml"
SQLITE_PATH="$TMP_DIR/investment-agent-p73.db"
VECLITE_PATH="$TMP_DIR/veclite.json"
SERVER_LOG="$TMP_DIR/server.log"
WEB_LOG="$TMP_DIR/web.log"
SERVER_PORT="${P73_SERVER_PORT:-18093}"
WEB_PORT="${P73_WEB_PORT:-14193}"
SERVER_BIN="$TMP_DIR/server"
ARTIFACT_DIR="${P73_ARTIFACT_DIR:-$ROOT_DIR/docs/release/ui-audit-assets/2026-06-19-p73}"
EFFECT_LOG="$ARTIFACT_DIR/effect-replay-check.log"
SERVER_PID=""
WEB_PID=""
...
P73_CAPTURE_SCREENSHOTS=1 \
P73_ARTIFACT_DIR="$ARTIFACT_DIR" \
E2E_BASE_URL="http://127.0.0.1:$WEB_PORT" \
npm --prefix "$ROOT_DIR/web" run test:e2e -- --config playwright.config.ts --workers=1 p73-product-effectiveness-ux-validation.spec.ts

python3 "$ROOT_DIR/scripts/p73_effect_replay_check.py" "$SQLITE_PATH" "$ARTIFACT_DIR" 2>&1 | tee "$EFFECT_LOG"
